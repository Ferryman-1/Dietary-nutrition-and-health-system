	...
	<!-- Top box -->
		...
		<div class="placeholder">
			...
			<div class="tm-paging-links">
				<nav>
					<form action="selectUserback" class="forms" id="form1" method="post" >
					<input type="hidden" name="pageNow" id="pageNow"/>
					状态：<select name="status">
	  					<option value="">请选择状态</option>
	  					<option value="0" <c:if test="${requestScope.userback.status==0 }">selected</c:if>>待受理</option>
	  					<option value="1" <c:if test="${requestScope.userback.status==1 }">selected</c:if>>已解决</option>
	  				</select>
					内容：<input class="input1" type="text" name="content" value="${requestScope.userback.content}"> 
    		...
    		<table class="tables" align="center">
					...
					<c:forEach items="${requestScope.userbackList}" var="userback">
						<tr class="status_open" align="center">
							<td><center><input type="checkbox" name="ids" value="${userback.id }"></center></td>
							
							<td>
							<c:forEach items="${requestScope.userList}" var="user">
							
								
								<c:if test="${user.id==userback.user.id}">${user.username }</c:if>
								
							</c:forEach>
							</td>
							<td>
								<div >${userback.email }</div>
							</td>
							<td  id="c">
								<div class="node">${userback.content }</div>
							</td>
							<td>
								<c:if test="${userback.status==0}">待受理</c:if>
	  							<c:if test="${userback.status==1}">已解决</c:if>
							</td>
							<td>
							...
							<td>
									<input type="button" onclick="location.href = 'updateUserbackView?id=${userback.id }'" id="buttons" value="处理"/>
								
									<input type="button" onclick="if (confirm('确定是否删除该反馈')) {  location.href = 'deleteUserbackById?id=${userback.id }'} " id="buttons" value="删除"/>				
