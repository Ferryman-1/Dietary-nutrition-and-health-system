	...
	<!-- Top box -->
		...
		<main>
			<header class="row tm-welcome-section">
				<h2 class="col-12 text-center tm-section-title">收藏帖子</h2>
				<p class="col-12 text-center">您所收藏的帖子都收录于此</p>
			</header>
			<div class="row tm-gallery">
				<!-- gallery page 1 -->
				
				<div id="tm-gallery-page-pizza" class="tm-gallery-page">
					<c:forEach items="${requestScope.collectChatList1}" var="collectChat">	${menu.id}
					<c:forEach items="${requestScope.chatList}" var="chat">
					<c:if test="${collectChat.chat_id==chat.id}">
					<article class="col-lg-3 col-md-4 col-sm-6 col-12 tm-gallery-item">
						<figure>
							<img src="img/lt/${chat.image_name }" alt="${chat.title }" class="img-fluid tm-gallery-img" />
							<figcaption>
								<h4 class="tm-gallery-title">
								<a href="selectChatById2?id=${chat.id }" class="tm-gallery-a">${chat.title }</a>
								<a class="dza" href="deleteCollectChatById3?id=${collectChat.id }">取消收藏</a>
								</h4>
							</figcaption>
						</figure>
					</article>
